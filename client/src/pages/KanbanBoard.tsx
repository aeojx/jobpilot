...
function JobCard({
  ...
  resumeStatusLoading?: boolean;
  isResumePolling?: boolean;
  onResumeGenerationStarted?: (jobId: number) => void;
}) {
      ...
      {(isToApply || job.status === "applied" || job.status === "nextsteps") && (
        ...
        >
          <ResumeButton
            ...
            isResumePolling={isResumePolling}
            onGenerationStarted={onResumeGenerationStarted}
          />
        </div>
      )}
...
function KanbanColumn({
  ...
  sortKey: SortKey;
  archiveCount?: number;
  resumeBatch: Record<number, ResumeStatusPayload | undefined>;
  resumeStatusLoading: boolean;
  resumePollJobIds: Set<number>;
  onResumeGenerationStarted: (jobId: number) => void;
}) {
      ...
      <div className="kanban-column-body">
        ...
        {sorted.map((job) => (
          <JobCard
            ...
            resumeStatusLoading={resumeStatusLoading}
            isResumePolling={resumePollJobIds.has(job.id)}
            onResumeGenerationStarted={onResumeGenerationStarted}
          />
        ))}
      </div>
    </div>
...
export default function KanbanBoard() {
  ...
  const resumeBoardJobIds = useMemo(
    () =>
      jobs
        .filter((j) => j.status === "to_apply" || j.status === "applied" || j.status === "nextsteps")
        .map((j) => j.id),
    [jobs]
  );
  const [resumePollExtraIds, setResumePollExtraIds] = useState<number[]>([]);
  const mergedResumeQueryIds = useMemo(() => {
    const s = new Set([...resumeBoardJobIds, ...resumePollExtraIds]);
    return [...s];
  }, [resumeBoardJobIds, resumePollExtraIds]);
        ...
        <div className="kanban-board-scroll flex gap-3 h-full" style={{ minWidth: "max-content" }}>
          {COLUMNS.map((col) => (
            <KanbanColumn
              ...
              resumeStatusLoading={resumeStatusLoading}
              resumePollJobIds={resumePollJobIds}
              onResumeGenerationStarted={handleResumeGenerationStarted}
            />
          ))}
        </div>
      </div>
